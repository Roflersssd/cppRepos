...
vector<Spending> LoadFromXml(istream& input) {
  vector<Spending> res;
  const std::vector<Node> nodes = Load(input).GetRoot().Children();
  for (auto spend : nodes) {
    res.push_back({spend.AttributeValue<string>("category"),
...
void TestXmlLibrary() {
  // РўРµСЃС‚ РґРµРјРѕРЅСЃС‚СЂРёСЂСѓРµС‚, РєР°Рє РїРѕР»СЊР·РѕРІР°С‚СЊСЃСЏ
  // Р±РёР±Р»РёРѕС‚РµРєРѕР№ РёР· С„Р°Р№Р»Р° xml.h
