...
class Database {
  ...
  bool hasId(const string& id) const { return byId.find(id) != byId.end(); }
 ...
 public:
  bool Put(const Record& record) {
    ...
    byTimestampSec.emplace(record.id,
                           byTimestamp.emplace(record.timestamp, record.id));
  ...
  void RangeByTimestamp(int low, int high, Callback callback) const {
    for (auto ptr = byTimestamp.lower_bound(low);
         ptr != byTimestamp.upper_bound(high); ptr++) {
