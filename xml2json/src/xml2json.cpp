...
Json::Document XmlToJson(const Xml::Document& doc) {
  ...
  for (const Xml::Node& n : doc.GetRoot().Children()) {
    Json::Node node(map<string, Json::Node>{
...
void TestJsonToXml() {
  const Json::Document json_doc{Json::Node(vector<Json::Node>{
      Json::Node(map<string, Json::Node>{{"category", Json::Node("food")},
                                         {"amount", Json::Node(2500)}}),
      Json::Node(map<string, Json::Node>{{"category", Json::Node("transport")},
                                         {"amount", Json::Node(1150)}}),
      Json::Node(
          map<string, Json::Node>{{"category", Json::Node("restaurants")},
                                  {"amount", Json::Node(5780)}}),
      Json::Node(map<string, Json::Node>{{"category", Json::Node("clothes")},
                                         {"amount", Json::Node(7500)}}),
      Json::Node(map<string, Json::Node>{{"category", Json::Node("travel")},
                                         {"amount", Json::Node(23740)}}),
      Json::Node(map<string, Json::Node>{{"category", Json::Node("sport")},
